...
-- ```
CREATE OR REPLACE FUNCTION automanage_updated_at(_tbl regclass)
  RETURNS VOID AS $$
BEGIN
  EXECUTE format('CREATE TRIGGER set_updated_at BEFORE UPDATE ON %s
                    FOR EACH ROW EXECUTE PROCEDURE autoset_updated_at()', _tbl);
END;
$$
LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION autoset_updated_at()
  RETURNS trigger AS $$
BEGIN
  IF (
...
INSERT INTO date_format (id, c_format, date_picker_format, js_format) VALUES
  (1, 'd/M/Y', 'dd/M/yyyy', 'DD/MMM/YYYY'),
...
INSERT INTO datetime_format (id, c_format, js_format) VALUES
  (1, 'd/M/Y g:i a', 'DD/MMM/YYYY h:mm:ss a'),
...
INSERT INTO timezone (id, name, gmt_offset, location) VALUES
  (1, 'Pacific/Midway', '-11:00', 'Midway Island'),
...
INSERT INTO currency (id, code, symbol, name, precision, format) VALUES
  (1, 'USD', '$', 'US Dollar', 2, '###,##.##'),
...
CREATE TABLE app_user (
  ...
  CONSTRAINT uk_app_user__login UNIQUE (username),
  CONSTRAINT uk_app_user__phone_number UNIQUE (phone_number)
);
...
CREATE TABLE user_password (
  ...
  CONSTRAINT pk_user_password PRIMARY KEY (user_id),
  CONSTRAINT fk_user_password__app_user FOREIGN KEY (user_id) REFERENCES app_user (id)
);
